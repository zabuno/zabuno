...
| 2.8 | **Yardım merkezi P0 makaleleri.** | ◐ **Dokuz makale yayında** (FF-230). `/help` giriş makalesi olarak KALDI (panelin ilk-kez ipuçları onun çıpalarına bağlı) ve artık bir makale dizini taşıyor; sekiz yeni makale `/help/<slug>` altında, her biri LİTERAL bir rota — desenli yazılsalardı statik dışa aktarıma ve 320 piksel ölçümüne hiç girmezlerdi. Konular `docs/106` yardım ağacının P0 satırlarından türetildi: hesap kurulumu, yayınlama/sorun giderme, masa ve QR, tasarım ve medya, menü yönetimi, ekip ve roller, misafir puanları, hesap ve faturalandırma. **Her makale tek bir soruya cevap verir, başlığı o sorudur ve NE YAPILAMADIĞINI da yazar** — üçü de testle bağlı. Ekran adları hafızadan değil arayüz kataloğundan (`lang/po/*.en.po`) ÖLÇÜLÜYOR: `HELP-CENTRE-REAL-LABELS-06` makalede kalın yazılan her adı katalogda arar, yani bir etiket değiştiğinde yardım sessizce yanlış kalmaz. 320 pikselde gerçek Chrome ile ölçüldü (`scripts/mobile-ux-audit`, sayfa kipi): yeni sayfalardan gelen bulgu SIFIR; kalan bulgular kabuk gezintisinin dondurulmuş borcu ve taban dışa aktarımda da var. **Eksik:** makaleler yalnız kaynak dilde (İngilizce) — çeviri kilidi açık değil ve her makale kaynak dilde olduğunu SÖYLÜYOR; `docs/106`'nın `/tr/yardim/` ağacı hâlâ kütükte `planned` (Faz 2.1/2.2 işi). **Yazılmayan iki konu:** çoklu dil (misafir dil seçimi var ama "daha fazla dil ekle" bugün tutulamayan bir söz) ve fotoğraftan menü okuma (Media yükleme listesinde `menuImportSource` slotunun İngilizce etiketi YOK, ham anahtar görünüyor — olmayan bir ad tarif edilemezdi). |

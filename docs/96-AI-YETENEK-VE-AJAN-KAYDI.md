...
| **Sıradaki iş — Gemini görüntü adaptörü** | Gemini (model adı henüz `config/ai.php`'ye yazılmadı — superadmin/MASTER karar verecek, öneri aşağıda) | Platform-owned tek Gemini hesabı (kasada zaten var) | aynı ekran, **birincil** aday (OpenAI'dan önce, `docs/51` §4b.1) | `VisionExtractionPort`, aynı onay zinciri |
| Çeviri taslağı (`opt-22`, henüz yazılmadı) | **Öneri: Gemini Flash sınıfı** (kısa metin, marka sesi orta risk) — Anthropic kasada olmadığı için Claude bu fazda **seçenek değil** | Platform-owned tek Gemini hesabı | Menü > Çeviri sekmesi | `StructuredGenerationPort`, insan düzenler/onaylar |
| Ürün açıklaması taslağı (`opt-23`, henüz yazılmadı) | **Öneri: Gemini Flash sınıfı** — alerjen alanına asla yazmaz | Platform-owned tek Gemini hesabı | Ürün düzenleme formu | `StructuredGenerationPort`, insan onaylı |
| Taksonomi yinelenen-terim tespiti | **Yerel gömme modeli** (`vps-ai` profili, model adı henüz seçilmedi — `docs/51` §4.3 yordamı: açık lisans + Türkçe fixture eval'i geçmeli) | Hesap **yok** — yerelde çalışır, bulut kimlik bilgisi gerektirmez | Arka planda, Menü Kataloğu | `EmbeddingPort`, insan onayı gerekmez (öneri, otomatik uygulama değil) |

...
**Amaç**: Basic QR Designer'ın (MVP) ötesinde şablon kütüphanesi, gelişmiş
frame/CTA seçenekleri sunmak.
**Bounded context**: QR Print Export modülünün üzerine kurulur.
**Owner**: Design + Engineering. **Sınıf**: Optional (M1).
**Bağımlılıklar**: QR Print Export.
**Public contracts/events**: `QRTemplateApplied`.
**Tenant isolation**: QR Print Export ile aynı.
**Permissions**: `qr.design.manage` (aynı izin, genişletilmiş yetenek).
**Entitlement**: Plan'a bağlı.
**ECA hooks**: Yok.
**AI-off/on**: AI şablon önerebilir; son seçim kullanıcıda.
**UX**: Şablon galerisinden tek tık uygulama.
**States**: Yok.
**Retention**: QR Print Export ile aynı.
**Observability**: Şablon kullanım popülerliği.
**Security**: Server-side scannability kapısı burada da geçerli (`docs/08` §4)
— gelişmiş tasarım okunabilirliği bozamaz.
**A11y/i18n**: Standart.
**Phase delivery**: Stage 2 Post-MVP.
**Acceptance**: Her şablonun scannability testinden geçtiğinin doğrulaması.
**Rollback**: Disable edilirse Basic Designer'a döner.
**Open questions**: Yok.
...
- **Optional AI use case(ler)**: Gelişmiş QR tasarım/renk/logo yerleşim önerisi
...
- **Data classification**: Tasarım meta verisi
- **Allowed tools/side effects**: Yalnız yukarıdaki opsiyonel kullanım
  ...
  dışına çıkmaz.
- **Forbidden authority (final-authority)**: Export/yayın kararı insan eylemidir

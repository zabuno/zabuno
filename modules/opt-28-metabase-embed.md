# OPT-28 — Metabase Embed

**PLANNING ONLY. Şu an çalıştırılamaz.**

**Amaç**: Gelişmiş/özelleştirilebilir raporlama için Metabase'i signed embed +
tenant row-level security ile entegre etmek.
**Bounded context**: Analytics/Consent/Tagging'in veri kaynağını **görselleştiren
opsiyonel dış katman** — kaynak veri ledger'ı Metabase'e kopyalanmaz, salt
okunur bağlanır.
**Owner**: Engineering. **Sınıf**: Optional (Growth+, koşullu — ek altyapı
gerektirir, shared-host varsayılanı değil, `docs/12` §5).
**Bağımlılıklar**: Analytics/Consent/Tagging, CORE-06 (embed secret).
**Public contracts/events**: `MetabaseEmbedRequested`.
**Tenant isolation**: Row-level security ile her tenant yalnız kendi verisini
görür — Metabase tarafında da tenant-scope zorunlu.
**Permissions**: `analytics.metabase.view`.
**Entitlement**: Enterprise/Growth üst segment.
**ECA hooks**: Yok.
**AI-off/on**: AI'dan bağımsız.
**UX**: Dashboard içinde signed iframe embed.
**States**: Yok.
**Retention**: Metabase kendi önbelleğini tutar, kaynak veri CORE-15
politikasına tabi kalır.
**Observability**: Embed yükleme süresi.
**Security**: Signed URL süresi sınırlı, tenant-scope ihlali testi zorunlu.
**A11y/i18n**: Embed çerçevesi kendi erişilebilirlik sınırlarına tabi (üçüncü
taraf UI).
**Phase delivery**: Growth Stage.
**Acceptance**: Bir tenant'ın signed embed URL'sinin başka tenant verisini
göstermediğinin testi.
**Rollback**: Disable edilirse temel Analytics/Consent/Tagging raporlaması
devam eder.
**Open questions**: Yok.
...
- **ai_posture**: advisory
- **Optional AI use case(ler)**: Dashboard veri anlatımı (narrative insight, RLS sınırları içinde)
...
- **Data classification**: Agregat rapor verisi
- **Allowed tools/side effects**: Yalnız yukarıdaki opsiyonel kullanım
  ...
  dışına çıkmaz.
- **Forbidden authority (final-authority)**: RLS/embed erişim kararı deterministik yetkilendirmededir

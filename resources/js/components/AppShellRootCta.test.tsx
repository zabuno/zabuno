...
describe('AppShell — public product home (S1-WP01A)', () => {
    ...
    it('renders a public-site navigation with discoverable in-page links', () => {
        ...
        expect(within(nav).getByRole('link', { name: /features/i })).toHaveAttribute(
            'href',
        ...
        expect(within(nav).getByRole('link', { name: /pricing/i })).toHaveAttribute(
            'href',
            expect.stringMatching(/^#/),
        );
    ...
    it('exposes an accessible link to open the workspace app at /app', () => {
        ...
        expect(screen.getByRole('link', { name: 'Open workspace app' })).toHaveAttribute(
            'href',
            '/app',
        );
    });
    ...
    it('renders a pricing section that honestly states no published plan prices or checkout exist yet', () => {
        ...
        expect(
            screen.getByText(
                /no (published )?(plan )?prices?.*(checkout|yet)|checkout.*not (yet )?available/i,
            ),
        ).toBeInTheDocument();
    });
    ...
    it('renders a contact section that honestly states there is no connected contact form yet', () => {
        ...
        expect(
            screen.getByText(
                /no connected contact form|contact form.*not (yet )?(connected|available)/i,
            ),
        ).toBeInTheDocument();
    });
    ...
    it('does not fabricate testimonial, trusted-customer, or percentage social-proof metrics', () => {
        ...
        expect(
            screen.queryByText(/\d+%\s*(faster|increase|growth|satisfaction|uptime)/i),
    ...
    it('describes setup and media honestly, without claiming unavailable invitation or menu-item attachment', () => {
        ...
        expect(screen.getByText(/workspace and restaurant setup/i)).toBeInTheDocument();

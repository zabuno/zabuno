 ...
 * BrandOnboardingForm.stories.tsx currently exports only Default and
 ...
 * EmptyNotApplicable / EditingNotApplicable declaration when a state does
 * not semantically apply to a create-only onboarding form.
 */
...
describe('BrandOnboardingForm.stories — Storybook contract matrix', () => {
    it('exports a meta pointing at BrandOnboardingForm with a Macro/Workspace title', async () => {
        ...
        expect(mod.default.component).toBe(BrandOnboardingForm);
    ...
    it('Success story provides a stateful render/play harness that surfaces the server-created brand after the onCreated callback flow', async () => {
        ...
        // output surface (the created brand's name) must not be visible.
        expect(() => getByText('Zeytin Restoranları')).toThrow();

        await mod.Success.play!({ canvasElement: container });

        expect(getByText('Zeytin Restoranları')).toBeDefined();
    });
});

...
type SupportedTypesTableProps = {
    ...
    accept: string;
    /** Sunucunun bildirdiği azami dosya boyutu; bilinmiyorsa `null`. */
 ...
 *
 * Azami boyut SUNUCUDAN gelir. Kaynakta "25 MB" yazıyor, bu depoda sunucu
 * 30 MB kabul ediyor; sabit yazılsaydı iki sayı bir gün ayrışır ve yanlış
 * olan ekranda kalırdı.
 */
export function SupportedTypesTable({ accept, maxBytes }: SupportedTypesTableProps) {
                    ...
                    {groups.map((group) => (
                        <tr key={group.key} className="border-t border-border align-top">
                            ...
                            <td className="p-[var(--space-2)] text-meta text-fg-muted tabular-nums">
                                {formatBytes(maxBytes ?? group.fallbackMaxBytes)}
                            </td>

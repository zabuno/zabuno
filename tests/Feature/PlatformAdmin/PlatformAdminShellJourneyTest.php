 ...
 * point (S1-WP01A follow-up). At authoring time there is no GET /platform
 * route in routes/web.php and no resources/js/platform.tsx Vite entry, so
 ...
 *
 * Frozen contract:
 * - GET /platform sits behind the same guard convention as GET /app:
 ...
 *   id="platform-admin-app" and a reference to the resources/js/platform.tsx
 ...
 */
final class PlatformAdminShellJourneyTest extends TestCase
{
    use RefreshDatabase;

    private const PLATFORM_URI = '/platform';
    ...
    private function grantTenantOwnerMembership(User $user): void
    {
        $workspaceId = DB::table('workspaces')->insertGetId([
            ...
            'slug' => 'platform-shell-test-workspace-'.$user->getKey(),
            'state' => 'active',
    ...
    public function test_guest_is_rejected_from_platform_route_not_with_a_bare_404(): void
    {
        ...
        self::assertNotSame(404, $response->getStatusCode(), 'GET /platform: rota var olmalı; 404 kabul edilmez.');
        self::assertContains($response->getStatusCode(), [302, 401], 'GET /platform: kimliksiz istek 302 (login redirect) veya 401 ile reddedilmeli.');
    }
    ...
    public function test_unverified_authenticated_user_is_rejected_from_platform_route(): void
    {
        ...
        self::assertNotSame(404, $response->getStatusCode(), 'GET /platform: rota var olmalı; 404 kabul edilmez.');
        self::assertContains($response->getStatusCode(), [302, 403], 'GET /platform: doğrulanmamış kullanıcı 302 (verify-notice redirect) veya 403 ile reddedilmeli.');
    }
    ...
    public function test_verified_super_admin_receives_the_platform_admin_shell(): void
    {
        ...
        $response->assertSee('id="platform-admin-app"', false);
        $response->assertSee('platform.tsx', false);
